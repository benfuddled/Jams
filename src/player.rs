...
pub fn gobbo() -> Result<i32> {
    const placeholder: bool = true;
    const placeholder_false: bool = false;
    let path = Path::new("/home/ben/Projects/YAMP/res/sample.flac");
    ...
    let format_opts =
        FormatOptions { enable_gapless: placeholder, ..Default::default() };
    ...
    match symphonia::default::get_probe().format(&hint, mss, &format_opts, &metadata_opts) {
        Ok(mut probed) => {
            ...
            if placeholder_false {
                ...
                let decode_opts = DecoderOptions { verify: placeholder, ..Default::default() };
